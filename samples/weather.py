...
def drawWeather(city):
    cityInfo = requests.get(API_BASE + '/api/location/search/?query=' + city).json()[0]
    cityWoeid = cityInfo['woeid']
    weather = requests.get(API_BASE + '/api/location/' + str(cityWoeid) + '/').json()['consolidated_weather'][0]

    app.group.clear()
    Label('Weather in ', 200, 30, size=40)
    Label(cityInfo['title'], 200, 75, size=40)
    Label(weather['weather_state_name'], 200, 130, size=30)
    imageURL = API_BASE + '/static/img/weather/png/'+ weather['weather_state_abbr'] + '.png'
    Image(imageURL, 200, 230, align='center', width=150, height=150)

    farenheitTemp = celciusToFarenheit(weather['the_temp'])
    Label(str(pythonRound(farenheitTemp, 2)) + '°F', 25, 350, align='left', size=30)
    Label('Wind: ' + weather['wind_direction_compass'], 375, 350, align='right', size=30)
