...
def fromPythonAngle(radians): return (90 - math.degrees(radians)) % 360
def toPythonAngle(degrees): return (math.radians(90 - degrees)) % (2 * math.pi)
...
def internalError(err):
    raise Exception(f'Internlal Error: {err}')
...
def rounded(n, precision = 0):
    if isinstance(n, list) or isinstance(n, tuple): return list(map(lambda v: rounded(v, precision), n))
    elif not (isinstance(n, int) or isinstance(n, float)): return n
    elif n < 0: return -rounded(-n, precision)
...
def roundedTupleString(a, precision = 0):
    return tupleString(rounded(a, precision))
...
def getArcPoints(cx, cy, width, height, startAngle = None, sweepAngle = None, sizeForN = None):
    ...
    if sweepAngle is None:
        ...
        pts.append([cx, cy])
    n = round(6 + 18 * sizeForN / 50)
    n = math.ceil(n / 4) * 4
    denominator = n if sweepAngle == 360 else n - 1
    startAngle = math.radians(90 - startAngle)
...
def round2(value):
    return round((value + 0.001) * 100) / 100
...
def getLinePoints(x1, y1, x2, y2, lineWidth):
    ...
    cx = (x1 + x2) / 2
    cy = (y1 + y2) / 2
    a = math.radians(angleTo(x1, y1, x2, y2) - 90) # -90 so we're off the horizontal
...
def getRegularPolygonPoints(cx, cy, r, points, rotateAngle):
    ...
    for i in range(1, points):
        ...
        pts.append([x, y])
    if rotateAngle: pts = rotatePoints(pts, math.radians(rotateAngle), cx, cy)
    return pts
