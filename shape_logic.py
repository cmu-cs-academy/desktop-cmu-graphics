...
def checkArgCount(clsName, fnName, argNames, args):
    if (len(argNames) != len(args)):
        ...
        pyThrow(f'Arg Count Error: {callSpec}() takes {len(argNames)} arguments ({",".join(argNames)}), not {len(args)}')
...
def checkRange(obj, attr, value, lo, hi):
    if ((value < lo) or (value > hi)): typeError(obj, attr, value, f'number-in-range-{lo}-{hi}')
...
def checkColor(obj, attr, value):
    ...
    pyThrow(f'Type Error: {objName} {attr} should be a color, and {value} is not a legal color name')
...
def checkAlign(obj, attr, value):
    ...
    if not value in aligns:
        pyThrow(f'Type Error: in {obj.__class__.__name__} {attr}, {value} is not a legal align value')
...
def toColorObject(v):
    ...
    raise Exception(f'toColorObject: unknown color type: {type(v)}')
...
def RGBListAlmostEqual(v1, v2, epsilon):
    ...
    for i in range(len(v1)):
        ...
        if isinstance(v1[i], RGB):
            ...
            raise Exception(f'RGBListAlmostEqual: invalid type: {v1[i]}')
    return True

def reflectGradientStart(start):
    def toggle(s, a, b):
        if a in s and b in s: raise Exception(f'Illegal start format: {s}')
...
class RGB(object):
    ...
    def set(self, attr, value):
        ...
        attrs = self.attrs
        attrs[attr] = value
        attrs['strVal'] = f"rgb({self.get('red')}, {self.get('green')}, {self.get('blue')})"
...
class Gradient(object):
    def __init__(self, colors, start):
        ...
        if len(colors) < 2:
            pyThrow(f'Need to pass at least 2 colors to gradient(); you gave {len(colors)}')
        for color in colors:
            if color is None:
                pyThrow('Type Error: None cannot be used inside gradient.colors')
            if isinstance(color, Gradient):
                pyThrow(f'Type Error: {color} cannot be used inside gradient.colors')
        checkString(self, 'start', start)
        if start not in gradientStarts:
            pyThrow(f'Illegal gradient start {start}')
        self.attrs = {'class': self.__class__.__name__, 'colors': colors, 'start': start}

    def toString(self):
        def quoted(s): return f"'{s}'"
        return f"gradient({'.'.join(v.toString() if isinstance(v, RGB) else quoted(v) for v in self.colors)}), start='{self.start}'"
...
class Shape(object):
    ...
    def drawDbPoint(self, ctx, x, y, color):
        ctx.save()
        self.setFillOrStrokeStyle(ctx, color)
        r = 3
        ctx.new_path()
    ...
    def drawDbBox(self, ctx):
        ...
        self.setFillOrStrokeStyle(ctx, 'red')
        ctx.set_dash([2, 2])
        ctx.stroke()
        ctx.restore()
    ...
    def draw(self, ctx):
        ...
        if (self.isGroup):
            ...
            if isinstance(self, Label):
                [targetX, targetY] = self.getApproxPoints()[0] # target start,top of text
                # rotate around targetX, targetY
                if self.rotateAngle != 0:
                    ctx.translate(targetX, targetY)
                    ctx.rotate(math.radians(self.rotateAngle))
                    ctx.translate(-targetX, -targetY)
...
class Group(Shape):
    ...
    def scalexy(self, varName, k, scaleAnchor = None):
        if (k == 1): return
        checkPositive(self, f'scale{varName}', k)
        ...
        for shape in self._shapes:
            if val is None:
                ...
                if ((attr == 'fill' and not colorTest(getattr(shape, attr), val, 0.005))
                    ...
                    pyThrow(f"Group.{attr} has no value because its children don't all have the same value for {attr}")
        return val
    ...
    def noPTA(self, attr):
        pyThrow(f'Group.{attr} cannot be read or modified')
...
class Label(Shape):
    ...
    def doRotate(self, degrees, cx, cy):
        newCenter = utils.rotatePoint([self.centerX, self.centerY], math.radians(degrees), cx, cy)
        self.set({
            'centerX': newCenter[0],
            'centerY': newCenter[1]
        })
    ...
    def setDims(self):
        ...
        pts = [[x0, y0], [(x0 + x1) / 2, y0], [x1, y0],
               ...
               [x0, (y0 + y1) / 2]]
        a = self.rotateAngle
        if a: pts = utils.rotatePoints(pts, math.radians(a), self.centerX, self.centerY)
        self.set({
            'approxPoints': pts,
    ...
    def createBaseGradient(self, gradient):
        ...
        if (start == 'center'):
            ...
            [[cx, cy]] = utils.rotatePoints([[cx, cy]], math.radians(-self.rotateAngle), targetX, targetY);
    ...
    def toString(self):
        return f'Label({self.value},{self.centerX},{self.centerY})'
...
class Polygon(Shape):
    ...
    def addxy(self, varName, d):
        ...
        checkNumber(self, f'add{varName}', d)
    ...
    def doRotate(self, degrees, cx, cy):
        self.pointList = utils.rotatePoints(self.pointList, math.radians(degrees), cx, cy)
    ...
    def toString(self):
        ...
        return f'Polygon{utils.roundedTupleString(args, 2)}'
    ...
    def createBaseGradient(self, fillOrBorder):
        ...
        if self.rotateAngle != 0:
            unrotatedPoints = utils.rotatePoints(
                self.pointList, math.radians(-self.rotateAngle), rotateAnchor[0], rotateAnchor[1]
            )
        ...
        if self.rotateAngle != 0:
            [[x0, y0], [x1, y1]] = utils.rotatePoints([[x0, y0], [x1, y1]], math.radians(self.rotateAngle),
...
class Rect(Polygon):
    ...
    def toString(self):
        ...
        return f'Rect{utils.roundedTupleString(args, 2)}'
...
class Line(Polygon):
    ...
    def toString():
        ...
        return f'Line{utils.roundedTupleString(args, 2)}'
...
class RegularPolygon(PolygonInCircle):
    ...
    def toString(self):
        ...
        return f'RegularPolygon{utils.roundedTupleString(args, 2)}'
...
class Star(PolygonInCircle):
    ...
    def toString(self):
        args = [self.centerX, self.centerY, self.radius, self.points]
        return f'Star{utils.roundedTupleString(args, 2)}'
...
class CMUSound(object):
    ...
    def __init__(self, url):
        self.soundProcess = subprocess.Popen([sys.executable, 'sound.py'], stdout=subprocess.PIPE, stdin=subprocess.PIPE)
...
class CMUImage(PolygonWithTransform):
    ...
    def toString(self):
        args = [self.left, self.top, self.width, self.height]
        return f'Image{utils.roundedTupleString(args, 2)}'
...
class Oval(PolygonWithTransform):
    ...
    def getBezierPoints(startAngle, sweepAngle):
        offset = math.radians(startAngle)
        remaining = sweepAngle
        bp = []
        while (remaining > 0):
            bp.extend(Oval.getBezierFragment(math.radians(min(90, remaining)), offset))
            offset += math.pi / 2
            remaining -= 90
        return bp
    ...
    def toString(self):
        ...
        return f'Oval{utils.roundedTupleString(args, 2)}'
...
class Circle(Oval):
    ...
    def toString(self):
        ...
        return f'Circle{utils.roundedTupleString(args, 2)}'
...
class ShapeLogicInterface(object):
    ...
    def slGet(self, slObj, attr):
        if not hasattr(slObj, attr):
            pyThrow(f'No such attribute: {attr}')
    ...
    def slInitShape(self, clsName, argNames, args, kwargs):
        ...
        for attr in kwargs:
            if shapeAttrs.get(attr, None) is None:
                pyThrow(f'"{attr}" is not a valid shape constructor argument')
