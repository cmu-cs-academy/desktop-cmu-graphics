...
def pyThrow(err): raise CMUException(err) from None
...
def checkNumber(obj, attr, value, isFn):
    if (type(value) != int and type(value) != float) or (type(value) == float and math.isnan(value)):
...
def checkGradientColor(value):
    if (value.__class__.__name__ == 'rgb'): return
    if (type(value) == str):
        if toEnglish(value, 'color').lower() not in CSS3_COLORS_TO_RGB:
            raise InvalidColorError(value)
        return
    raise WrongTypeError(value, 'color string or rgb')

def pythonColorToSlColor(value, allowNone = False):
    if (type(value) == str):
        if toEnglish(value, 'color').lower() not in CSS3_COLORS_TO_RGB:
            raise InvalidColorError(value)
        return value

    if (hasattr(value, '_sl') and isinstance(value._sl, (RGB, Gradient))):
        return value._sl

    if (allowNone and value == None):
        return None

    raise WrongTypeError(value, 'color')

def checkType(value, t, typeName):
    if type(value) == t:
        return
    raise WrongTypeError(value, typeName) 
...
def checkStringFuture(value):
    checkType(value, str, 'string')
...
def checkGradientStart(value):
    checkStringFuture(value)
    if (
        toEnglish(value, 'gradient-start') not in gradientStarts and
        toEnglish(value, 'gradient-start') not in alternateGradientStarts.keys()
    ):
        raise InvalidGradientStartError(value)
...
class InternalTypeError(Exception):
    ...
    def doRaise(self, *args):
        pyThrow(self.getMessage(*args))

class WrongTypeError(InternalTypeError):
    def __init__(self, value, correctType):
        super().__init__(value)
        self.correctType = correctType;

    def getMessage(self, callSpec, *args):
        return t(
            '{{error}}: {{callSpec}} should be {{typeName}} (but {{value}} is of type {{valueType}})',
            {
                'value': repr(self.value),
                'valueType': self.value.__class__.__name__,
                'typeName': t(self.correctType),
                'error': t('TypeError'),
                'callSpec': callSpec,
            }
        )

class InvalidColorError(InternalTypeError):
  def getMessage(self, callSpec, *args):
    return t(
      '{{error}}: {{callSpec}} should be a color, and {{value}} is not a legal color name',
      {
...
class InvalidGradientStartError(InternalTypeError):
    def getMessage(self, *args):
        return t('{{error}}: Illegal gradient start ({{start}})', {
            'start': repr(self.value),
            'error': t('TypeError'),
        })

class WrongListLengthError(InternalTypeError):
    def __init__(self, value, errorMsg, length):
        super().__init__(value)
        self.errorMsg = errorMsg
        self.length = length

    def getMessage(self, callSpe, *args):
        return t(
            '{{error}}: {{callSpec}} - {{errorMsg}} (but {{value}} has length {{length}})',
            {
...
class Gradient(object):
    def __init__(self, colors, start):
        ...
        self.rgbColors = self.getRGBColors();
...
class Group(Shape):
    ...
    fill = property(get_fill, set_fill)
    def set__fill(self, v): return self.setPA('_fill', v)
    _fill = property(None, set_fill)
...
class ShapeLogicInterface(object):
    ...
    def rgb(self, red, green, blue):
        checkNumberInRange(t('rgb'), 'red', red, 0, 255, False)
        checkNumberInRange(t('rgb'), 'green', green, 0, 255, False)
        checkNumberInRange(t('rgb'), 'blue', blue, 0, 255, False)
        return RGB(red, green, blue)

    def gradient(self, colors, start):
        try:
            argName = 'colors'
            if (len(colors) < 2):
                raise WrongListLengthError(
                    colors,
                    'Should have at least two colors in gradient',
                    len(colors)
                )

            for color in colors:
                checkGradientColor(color)
            slColors = [pythonColorToSlColor(color) for color in colors]
            argName = 'start'

            checkGradientStart(start)
            return Gradient(slColors, start)
        except InternalTypeError as e:
            e.doRaise(f"{t('gradient')}.{t('colors')}")
    ...
    def slGet(self, slObj, attr):
        ...
        if (attr == 'fill' or attr == 'border'):
            if attr in slObj.attrs:
                attr = '_' + attr
    ...
    def slSetWithTypeCheck(self, obj, attr, val):
        ...
        className = 'Image' if objClass == 'CMUImage' else objClass
        try:
            if shapeAttrs.get(attr, None) is not None:
                if (attr == 'fill' or attr == 'border'):
                    setattr(obj, '_' + attr, val)
                    val = pythonColorToSlColor(val, True)
                else:
            ...
            e.doRaise(f'{t(className)}.{t(attr)}')
